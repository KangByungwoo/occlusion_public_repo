...
class UnoccludedMultiObjectClassifierExp(object):
    def __init__(self, args):
        ...
        if self.model_type == 'FD':
            self.model = FDMultiObjectClassifier(input_channels=self.n_objs, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'FDFat':
            self.model = FDMultiObjectClassifier(input_channels=self.n_objs, hidden1=128, hidden2=128, hidden3=128, hidden4=2048, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'RC':
            self.model = RCMultiObjectClassifier(input_channels=self.n_objs, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'RCReversed':
            self.model = RCMultiObjectClassifier(input_channels=self.n_objs, reverse=True, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'RCControl':
            self.model = RCControlMultiObjectClassifier(input_channels=self.n_objs, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'RCLast':
            self.model = RCLastMultiObjectClassifier(input_channels=self.n_objs, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'RC5':
            self.model = RCMultiObjectClassifier(input_channels=self.n_objs, n_iter=5, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'RCReversed5':
            self.model = RCMultiObjectClassifier(input_channels=self.n_objs, n_iter=5, reverse=True, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'RCLast5':
            self.model = RCLastMultiObjectClassifier(input_channels=self.n_objs, n_iter=5, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'RCLast5Uniform':
            self.model = RCLastMultiObjectClassifier(input_channels=self.n_objs, n_iter=5, n_classes=len(self.valid_classes), dropout_p=self.dropout_p, init_method='uniform')
        elif self.model_type == 'FDFatterTaller2':
            self.model = FDTallerMultiObjectClassifier(input_channels=self.n_objs, hidden=256, hidden_fc=1024, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)                 
        elif self.model_type == 'WeightSharedFDFatterTaller2':
            self.model = WeightSharedFDTallerMultiObjectClassifier(input_channels=self.n_objs, hidden=256, hidden_fc=1024, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'TDRCTypeC':
            self.model = TDRCMultiObjectClassifierC(input_channels=self.n_objs, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'TDRCTypeC5':
            self.model = TDRCMultiObjectClassifierC(input_channels=self.n_objs, n_iter=5, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'TDRCTypeD':
            self.model = TDRCMultiObjectClassifierD(input_channels=self.n_objs, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'TDRCTypeD5':
            self.model = TDRCMultiObjectClassifierD(input_channels=self.n_objs, n_iter=5, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for iter, (_, ind_objs, labels) in enumerate(self.train_loader):
                ...
                data = Variable(ind_objs.view(N, -1, H, W))
            ...
            self.loss_fct.eval()
            iter_start_time = time.time()
            for iter, (_, ind_objs, labels) in enumerate(self.val_loader):
                N, n_objs, C, H, W = ind_objs.size()
                data = Variable(ind_objs.view(N, -1, H, W), volatile=True)
    ...
    def best_hp_test(self, trial_idx, data_dir=None):
        ...
        kwargs = {'num_workers': 8, 'pin_memory': True} if self.gpu_mode else {}
        self.test_loader = DataLoader(utils.MultiObjectDataset(data_dir, data_type='test', requires_ind_objs=True), shuffle=True, batch_size=self.test_batch_size, 
            drop_last=False, **kwargs)
        ...
        self.loss_fct.eval()
        iter_start_time = time.time()
        for iter, (_, ind_objs, labels) in enumerate(self.test_loader):
            N, n_objs, C, H, W = ind_objs.size()
            data = Variable(ind_objs.view(N, -1, H, W), volatile=True)
            labels = Variable(labels, volatile=True)
            if self.gpu_mode:
