...
from occluded_single_object_classifier import *

plt.rcParams['figure.figsize'] = (10.0, 8.0) # set default size of plots
plt.rcParams['image.interpolation'] = 'nearest'
plt.rcParams['image.cmap'] = 'gray'

class OccludedSingleObjectClassifierExp(object):
    def __init__(self, args):
        ...
        if self.model_type == 'FD':
            self.model = FDOccludedSingleObjectClassifier(n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'FDFat':
            self.model = FDOccludedSingleObjectClassifier(hidden1=128, hidden2=128, hidden3=128, hidden4=2048, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'RC':
            self.model = RCOccludedSingleObjectClassifier(n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'RC5':
            self.model = RCOccludedSingleObjectClassifier(n_iter=5, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'RCControl':
            self.model = RCControlOccludedSingleObjectClassifier(n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'FDFatterTaller2':
            self.model = FDTallerOccludedSingleObjectClassifier(hidden=256, hidden_fc=1024, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)                 
        elif self.model_type == 'WeightSharedFDFatterTaller2':
            self.model = WeightSharedFDTallerOccludedSingleObjectClassifier(hidden=256, hidden_fc=1024, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)                 
        elif self.model_type == 'TDRCTypeC':
            self.model = TDRCOccludedSingleObjectClassifierC(n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'TDRCTypeC5':
            self.model = TDRCOccludedSingleObjectClassifierC(n_iter=5, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'TDRCTypeD':
            self.model = TDRCOccludedSingleObjectClassifierD(n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        elif self.model_type == 'TDRCTypeD5':
            self.model = TDRCOccludedSingleObjectClassifierD(n_iter=5, n_classes=len(self.valid_classes), dropout_p=self.dropout_p)
        ...
        if not self.debug:
            self.data_dir = os.path.join('extensive_info_including_fg_blacked_out_multi_' + self.data_type + self.data_rotate_flag + '_' + self.valid_classes_flag,
                                        '{}_{}_{}_{:.2f}_{:.2f}_{:.0e}_{:.0e}'.format(self.n_objs, self.in_size, self.out_size, self.vr_min, self.vr_max, self.eps, self.eps2))
        else:
            self.data_dir = os.path.join('extensive_info_including_fg_blacked_out_debugging_multi_' + self.data_type + self.data_rotate_flag + '_' + self.valid_classes_flag,
                                        '{}_{}_{}_{:.2f}_{:.2f}_{:.0e}_{:.0e}'.format(self.n_objs, self.in_size, self.out_size, self.vr_min, self.vr_max, self.eps, self.eps2))
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for iter, (data, labels) in enumerate(self.train_loader):
                ...
                total_loss = self.loss_fct(scores, labels[:,1])/len(data)
                total_loss.backward()
                self.optimizer.step()

                total_acc = (scores.max(1)[1] == labels[:,1]).sum().item()/len(labels)
                ...
                if ((iter + 1) % 100) == 0:
                    ...
                    print("Epoch {}: [{}/{}] total_loss: {:.3f} total_acc: {:.3f} ({:.3f} sec)".format(
                        (epoch + 1), (iter + 1), len(self.train_loader), total_loss.item(), total_acc, iter_time))
            ...
            for iter, (data, labels) in enumerate(self.val_loader):
                ...
                scores= self.model(data)
                total_loss += self.loss_fct(scores, labels[:,1]).item()/len(data)
                ...
                if self.vis_wrong_cls2:
                    ...
                    wrong_cls2_idx = torch.nonzero((scores.max(1)[1] != labels[:,1])).squeeze()
    ...
    def best_hp_test(self, trial_idx, data_dir=None):
        ...
        return total_acc, best_val_total_acc
