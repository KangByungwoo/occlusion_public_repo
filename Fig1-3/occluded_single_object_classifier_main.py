...
from occluded_single_object_classifier_exp import OccludedSingleObjectClassifierExp
import torch
import numpy as np
...
def parse_args():
    ...
    parser.add_argument('--save_dir', type=str, default='occluded_single_object_classifier_models')
    parser.add_argument('--result_dir', type=str, default='occluded_single_object_classifier_results')
    parser.add_argument('--log_dir', type=str, default='occluded_single_object_classifier_logs')
    parser.add_argument('--hp_dir', type=str, default='occluded_single_object_classifier_hp_searchs')
    parser.add_argument('--best_hp_test_dir', type=str, default='occluded_single_object_classifier_best_hp_test')
...
def check_args(args):
    ...
    if args.test:
        ...
        args.test_data_dir = os.path.join('extensive_info_including_fg_blacked_out_multi_' + args.test_data_type + test_data_rotate_flag + '_' + args.test_valid_classes_flag, 
...
def main():
    ...
    if args.best_hp_test:
        best_hps = {}
        best_hps['FD'] = {'lr': 9.2*10**(-4), 'weight_decay': 2.2*10**(-4), 'batch_size': 64, 'dropout_p': 0.31}
        best_hps['FDFat'] = {'lr': 6.4*10**(-4), 'weight_decay': 1.6*10**(-11), 'batch_size': 32, 'dropout_p': 0.46}
        best_hps['RC'] = {'lr': 9.0*10**(-4), 'weight_decay': 5.2*10**(-10), 'batch_size': 128, 'dropout_p': 0.48}
        best_hps['RCControl'] = {'lr': 1.4*10**(-3), 'weight_decay': 8.4*10**(-11), 'batch_size': 128, 'dropout_p': 0.27}
        best_hps['RC5'] = {'lr': 3.6*10**(-4), 'weight_decay': 1.8*10**(-10), 'batch_size': 32, 'dropout_p': 0.23}

        best_hps['FDFatterTaller2'] = {'lr': 1.0*10**(-4), 'weight_decay': 1.7*10**(-4), 'batch_size': 64, 'dropout_p': 0.42}
        best_hps['WeightSharedFDFatterTaller2'] = {'lr': 1.5*10**(-4), 'weight_decay': 8.8*10**(-11), 'batch_size': 128, 'dropout_p': 0.45, 'val_acc': 0.868}
        best_hps['TDRCTypeC'] = {'lr': 5.5*10**(-4), 'weight_decay': 1.1*10**(-8), 'batch_size': 64, 'dropout_p': 0.28}
        best_hps['TDRCTypeC5'] = {'lr': 6.4*10**(-4), 'weight_decay': 7.0*10**(-7), 'batch_size': 64, 'dropout_p': 0.34}
        best_hps['TDRCTypeD'] = {'lr': 8.7*10**(-4), 'weight_decay': 7.9*10**(-10), 'batch_size': 128, 'dropout_p': 0.49, 'val_acc': 0.822}
        best_hps['TDRCTypeD5'] = {'lr': 6.6*10**(-4), 'weight_decay': 4.7*10**(-10), 'batch_size': 128, 'dropout_p': 0.29, 'val_acc': 0.868}
        ...
        for model_type, hps in best_hps.items():
            ...
            while True:
                ...
                exp = OccludedSingleObjectClassifierExp(args)
                total_acc, val_total_acc = exp.best_hp_test(trial_idx=(trial_idx+1))
