...
def load_exact_background(bg_cat, bg_idx, bg_angle, bg_trans, reduced_data_dir, eps, eps2):
    """
    Reload EXACT same background single-object 28×28 from (bg_cat, bg_idx),
    apply angle=bg_angle, return (bg_img, bg_mask).
    """
    bg_28 = load_single_object_by_index(bg_cat, bg_idx, reduced_data_dir)  # EXACT
    bg_img, bg_mask = apply_transform(bg_28, bg_angle, bg_trans, eps, eps2)
...
def generate_x2_for_scenario(
    ...
    """
    Reconstruct EXACT same background from (bg_cat, bg_idx).
    If 'orientation' => use EXACT same FG instance from x1 (fg_cat, fg_idx), 
       but different angle.
    If 'category' => pick a new cat for fg, new random instance. 
       Keep orientation the same as x1 => angle2 = fg_angle1
    If 'both' => new cat, new instance, new angle.
    Returns x2, meta2
    """
    # EXACT background
    bg_cat = x1_meta['bg_cat']
    bg_idx = x1_meta['bg_idx']
    bg_angle = x1_meta['bg_angle']
    bg_trans = x1_meta['bg_trans']
    bg_img, bg_mask = load_exact_background(bg_cat, bg_idx, bg_angle, bg_trans, reduced_data_dir, eps, eps2)

    # Foreground info from x1
    fg_cat1 = x1_meta['fg_cat']
    fg_idx1 = x1_meta['fg_idx']
    fg_angle1 = x1_meta['fg_angle']
    fg_trans1 = x1_meta['fg_trans']
    x1_vr = x1_meta['x1_vr']

    fg_trans2 = fg_trans1

    if scenario == 'category':
        # same orientation => angle2 = fg_angle1
        angle2 = fg_angle1
        # different category => pick new cat
        fg_cat2 = random.choice([c for c in valid_classes if c != fg_cat1])
        # pick a random instance from that cat
        fg_28_2, fg_idx2 = load_single_object_random(fg_cat2, reduced_data_dir)
    elif scenario == 'orientation':
        # same cat => fg_cat2 = fg_cat1
        # same EXACT instance => reload (fg_cat1, fg_idx1)
        fg_cat2 = fg_cat1
        fg_idx2 = fg_idx1
        # different angle
        while True:
            candidate_angle = random.uniform(angle_min, angle_max)
            if abs(candidate_angle - fg_angle1) > math.pi/180*30:  # 30 degrees
                angle2 = candidate_angle
                break
    else:  # scenario='both'
        # different category => new cat
        fg_cat2 = random.choice([c for c in valid_classes if c != fg_cat1])
        # random instance from that cat
        fg_28_2, fg_idx2 = load_single_object_random(fg_cat2, reduced_data_dir)
        while True:
            candidate_angle = random.uniform(angle_min, angle_max)
            if abs(candidate_angle - fg_angle1)  > math.pi/180*30:  # 30 degrees
    ...
    # Now build the FG image for x2
    if scenario == 'orientation':
        # reload EXACT foreground instance => 
        # we do not pick from random, we do:
        fg_28_2 = load_single_object_by_index(fg_cat2, fg_idx2, reduced_data_dir)
    else:
        # category or both => we either loaded a random instance above 
        # or already have it in fg_28_2 from random. 
        # If scenario=='both' or 'category', we have (fg_28_2, fg_idx2).
        pass

    fg_img2, fg_mask2 = apply_transform(fg_28_2, angle2, fg_trans2, eps, eps2)

    x2 = compose_two_objects(bg_img, bg_mask, fg_img2, fg_mask2)


    # Compute visible ratio VR for n_objs=2:
    # intersection of both object masks:
    x2_intsec = bg_mask * fg_mask2
...
def evaluate_model_on_scenario_with_strategies(model, test_loader, device='cuda'):
    ...
    for x1_batch, x2_batch, bg_cat_batch in test_loader:
        x1_batch, x2_batch, bg_cat_batch = x1_batch.to(device), x2_batch.to(device), bg_cat_batch.to(device)
        ...
        pass1_correct_sum += (pass1_preds == bg_cat_batch).sum().item()
        ...
        for strategy_name, layers_to_replace in replace_strategies.items():
            ...
            pass2_correct_sum[strategy_name] += (pass2_preds == bg_cat_batch).sum().item()

        total_samples += bg_cat_batch.size(0)
...
def main_test_with_replacement_strategies(pair_data_dir, model_dir, results_file, plot_save_path):
    ...
    if os.path.exists(results_file):
        ...
        for model_file in models:
            ...
            model = RCMultiObjectClassifier(n_classes=4, dropout_p=0)
...
def plot_all_strategies_single_figure(
    ...
    # ax.grid(axis='y', linestyle='--', alspha=0.7)
    ax.legend(loc='best')
...
import matplotlib.pyplot as plt
import numpy as np
from scipy.stats import mannwhitneyu, ks_2samp # Importing the Mann-Whitney U test
...
def plot_occ_level_for_scenario(pair_data_dir, scenario, plot_dir):
    plot_save_path = os.path.join(plot_dir, 'plot_occ_level_for_scenario', f'{scenario}_occ_level_plot')
    os.makedirs(os.path.join(plot_dir, 'plot_occ_level_for_scenario'), exist_ok=True)
    ...
    # Determine the combined min and max for both datasets to synchronize bins
    combined_min = min(min(x1_vr_list), min(x2_vr_list))
    combined_max = max(max(x1_vr_list), max(x2_vr_list))
    ...
    # stat, p_value = mannwhitneyu(x1_vr_list, x2_vr_list, alternative='two-sided')
    stat, p_value = ks_2samp(x1_vr_list, x2_vr_list, alternative='two-sided')
    
    # Plot the distributions with the same bins
    plt.figure(figsize=(6, 4))
    plt.hist(x1_vr_list, bins=bins, alpha=0.7, label='Original images', color='blue', edgecolor='black')
    plt.hist(x2_vr_list, bins=bins, alpha=0.7, label='Transformed images', color='orange', edgecolor='black')
    
    # Add labels and title
    plt.xlabel('Occlusion Levels')
    plt.ylabel('Number of images')
    # plt.title(f'Distribution of x1 and x2 Occlusion Levels ({scenario})')
    plt.legend()
    ...
    # Save the plot
    plt.savefig(plot_save_path + '.png', dpi=300)
    plt.savefig(plot_save_path + '.svg', format='svg')
    plt.close()



def generate_sample_images_for_each_scenario_for_given_x1(
    num_samples=5,
    ...
    """
    Generates a single original image (x1) and creates a transformed image (x2)
    for each of the three scenarios. Then, plots x1 along with its three corresponding
    x2 images in one row and saves the figure as an SVG.

    Args:
        valid_classes (list): List of valid class labels.
        reduced_data_dir (str): Path to the directory with reduced data.
        angle_min (float): Minimum rotation angle (in radians).
        angle_max (float): Maximum rotation angle (in radians).
        eps (float): First threshold for mask refinement.
        eps2 (float): Second threshold for mask refinement.
    ...
    scenarios = ['category', 'orientation', 'both']
    x2_images = {}  # Dictionary to hold x2 for each scenario
    for sample_idx in range(num_samples):

        # Generate the original image x1 and its metadata.
        x1, meta1 = generate_single_x1(
            valid_classes=valid_classes,
        ...
        # For each scenario, generate x2 using the same x1 metadata.
        for scenario in scenarios:
            x2, meta2 = generate_x2_for_scenario(
                scenario=scenario,
                x1_meta=meta1,
            ...
            x2_images[scenario] = x2

        # Create a figure with 4 subplots in a single row:
        #   - Column 0: original x1
        #   - Column 1: x2 for scenario "category"
        #   - Column 2: x2 for scenario "orientation"
        #   - Column 3: x2 for scenario "both"
        fig, axes = plt.subplots(1, 4, figsize=(12, 3))

        # Plot the original x1.
        axes[0].imshow(x1.squeeze(0).numpy(), cmap='gray', interpolation='none')
        axes[0].set_title('Original x1')
        axes[0].axis('off')

        # Plot each transformed image x2.
        for idx, scenario in enumerate(scenarios, start=1):
            axes[idx].imshow(x2_images[scenario].squeeze(0).numpy(), cmap='gray', interpolation='none')
            axes[idx].set_title(f'Scenario:\n{scenario}')
            axes[idx].axis('off')

        plt.tight_layout()
        svg_output_path = os.path.join(output_dir, f'sample_images_{sample_idx}.svg')
        png_output_path = os.path.join(output_dir, f'sample_images_{sample_idx}.png')
        plt.savefig(svg_output_path, format='svg')
        plt.savefig(png_output_path, format='png')
        plt.close(fig)
        print(f"Saved sample image figure")



def main():
    pair_data_dir = './hidden_state_pert_test_for_multi_object_classifier_RC_pair_data'
    results_dir = './hidden_state_pert_test_for_multi_object_classifier_each_layer_RC_results'
    ...
    model_dir = './multi_object_classifier_best_hp_test/extensive_info_multi_fashion_mnist_rotated_-180_180_trans_0.10_0.15_0289/2_28_50_0.25_0.50_1e-02_1e-01/RC'
    ...
    do_generate_sample_images_for_each_scenario_for_given_x1 = False
    ...
    if do_generate_sample_images_for_each_scenario_for_given_x1:
        generate_sample_images_for_each_scenario_for_given_x1(num_samples=20,
            valid_classes=[0, 1, 2, 3],
            reduced_data_dir='./reduced_fashion_mnist/0289/test',
            angle_min=-math.pi,
            angle_max=math.pi,
            eps=1e-2,
            eps2=1e-1,
            output_dir=os.path.join(results_dir, 'generate_sample_images_for_each_scenario_for_given_x1'))
